...
import { Home, Apple, BookOpen, CalendarDays, ShoppingCart, NotebookText } from 'lucide-react'
import { cn } from '@/lib/utils/cn'

const navItems = [
  { href: '/dashboard', label: 'Главная', icon: Home },
  { href: '/products', label: 'Продукты', icon: Apple },
  { href: '/diary', label: 'Дневник', icon: NotebookText },
...
export function BottomNavigation() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-t border-border shadow-lg pb-safe md:hidden">
...
export function SidebarNavigation() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 min-h-screen bg-white border-r border-border p-4 fixed left-0 top-0 bottom-0">
      {/* Logo */}
      <div className="flex items-center gap-3 px-3 py-4 mb-6">
        <div className="w-10 h-10 rounded-2xl bg-primary flex items-center justify-center text-xl">
          🥣
        </div>
        <div>
